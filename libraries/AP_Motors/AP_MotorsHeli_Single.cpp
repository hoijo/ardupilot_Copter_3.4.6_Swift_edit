...
void AP_MotorsHeli_Single::move_actuators(float roll_out, float pitch_out, float coll_in, float yaw_out)
{
    ...
    float thrust_lower = - collective_out + (yaw_out + yaw_offset) * 0.5f;
    // update the yaw rate using the tail rotor/servo
    move_yaw(thrust_lower);
...
void AP_MotorsHeli_Single::write_aux(float servo_out)
{
    ...
    // rc_write(AP_MOTORS_HELI_SINGLE_AUX, calc_thrust_to_pwm(servo_out, _servo_aux));
}
